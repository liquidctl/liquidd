...
liquid_hid_device_class_init(LiquidHidDeviceClass* class)
{
    ...
    pspecs[PROP_MAX_INPUT_REPORT_SIZE] = g_param_spec_uint(
        "max-input-report-size", // name
    ...
    pspecs[PROP_INPUT_STREAM] = g_param_spec_object(
        "input-stream", // name
        "Input stream", // nick
    ...
    pspecs[PROP_OUTPUT_STREAM] = g_param_spec_object(
        "output-stream", // name
        "Output stream", // nick
    ...
    signals[SIGNAL_INPUT_REPORT] = g_signal_new(
        "input-report", // signal_name,
